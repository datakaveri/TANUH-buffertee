...
func VerifyOIDCToken(tokenString, expectedAudience string, leeway time.Duration) (*OIDCClaims, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("dispatch/verify_oidc: parse token: %w", err)
	}

	if !token.Valid {
		return nil, fmt.Errorf("dispatch/verify_oidc: token not valid")
	}

	claims, ok := token.Claims.(jwt.MapClaims)
	if !ok {
		return nil, fmt.Errorf("dispatch/verify_oidc: unexpected claims type")
	}

	// Manual exp check with leeway (jwt/v4 has no WithLeeway option).
	if !claims.VerifyExpiresAt(time.Now().Add(-leeway).Unix(), true) {
		return nil, fmt.Errorf("dispatch/verify_oidc: token expired")
	}

	iss, _ := claims["iss"].(string)
	if iss != GCPCSIssuer {
		return nil, fmt.Errorf("dispatch/verify_oidc: iss %q != expected %q", iss, GCPCSIssuer)
	}

	switch aud := claims["aud"].(type) {
	case string:
		if aud != expectedAudience {
			return nil, fmt.Errorf("dispatch/verify_oidc: aud %q != expected %q", aud, expectedAudience)
		}
	case []any:
		found := false
		for _, a := range aud {
		...
		if !found {
			return nil, fmt.Errorf("dispatch/verify_oidc: aud does not contain %q", expectedAudience)
		}
	default:
		return nil, fmt.Errorf("dispatch/verify_oidc: aud claim missing or wrong type")
	}

	eatNonce, err := extractEatNonce(claims)
	if err != nil {
		return nil, fmt.Errorf("dispatch/verify_oidc: %w", err)
	}

	hwmodel, _ := claims["hwmodel"].(string)
	if hwmodel == "" {
		return nil, fmt.Errorf("dispatch/verify_oidc: hwmodel claim missing")
	}
