...
app = Flask(__name__)
loopback_app = Flask(__name__ + ".loopback")
...
# KeyStore: job_id -> base64-encoded AES-128-GCM symmetric key
_KEY_STORE: dict = {}
_KEY_STORE_LOCK = threading.Lock()

LOOPBACK_PORT = int(os.getenv("KEY_STORE_LOOPBACK_PORT", "8081"))
...
def generate_symmetric_key():
    """Generate a 128-bit AES-GCM symmetric key, returned as base64."""
...
def store_key():
    """Receive { job_id, key_b64 } from the processing TEE (over TLS) and persist in KeyStore."""
    content = request.json or {}
    job_id = content.get("job_id")
    key_b64 = content.get("key_b64")
    if not job_id or not key_b64:
        return jsonify({"status": "error", "message": "Missing job_id or key_b64"}), 400
    with _KEY_STORE_LOCK:
        _KEY_STORE[job_id] = key_b64
    buffer_debug(f"KeyStore: stored key for job {job_id}")
    return jsonify({"status": "success", "job_id": job_id}), 200


@loopback_app.route("/api/get-key", methods=["GET"])
def loopback_get_key():
    """Return the AES-128-GCM key for a job_id (loopback-only, plaintext HTTP)."""
    job_id = request.args.get("job_id")
    if not job_id:
        return jsonify({"status": "error", "message": "Missing job_id parameter"}), 400
    with _KEY_STORE_LOCK:
        key_b64 = _KEY_STORE.get(job_id)
    if key_b64 is None:
        return jsonify({"status": "error", "message": "Key not found"}), 404
    return jsonify({"job_id": job_id, "key_b64": key_b64}), 200


def _start_loopback_server():
    from werkzeug.serving import make_server
    server = make_server("127.0.0.1", LOOPBACK_PORT, loopback_app)
    t = threading.Thread(target=server.serve_forever, daemon=True, name="loopback-key-server")
    t.start()
    buffer_debug(f"Loopback key-retrieval server started on 127.0.0.1:{LOOPBACK_PORT}")
...
if __name__ == "__main__":
    ...
    print("  - POST /api/store-key  (TLS, external — deposit key for a job)")
    print(f"  - GET  /api/get-key   (plaintext HTTP, 127.0.0.1:{LOOPBACK_PORT} loopback only)")
    print("=" * 60)
    run_kwargs = {
