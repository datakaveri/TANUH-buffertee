#!/bin/sh
# Stop the GPU Processing TEE VM. Used by the Buffer TEE between provisioning
# retries so each GPU attempt is a clean stop→start cycle.
set -eu
...
HTTP_CODE="$(curl -s -o /tmp/stop-gpu-cs-vm-response.json -w '%{http_code}' \
  -X POST \
...
echo "Response:"
cat /tmp/stop-gpu-cs-vm-response.json
echo
